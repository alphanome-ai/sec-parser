...
class TopSectionManagerFor10Q(AbstractElementwiseProcessingStep):
    ...
    Output:
    - element of type AbstractSemanticElement: the processed version (could have been
    converted) of the given semantic element "element".

    Raises:
    - ValueError: If the given iternation number is invalid. The allowed values of
    iteration numbers are 0 and 1.
    ...
    Functionality:
    - Checks if the elements text matches a part pattern by calling the match_part
    method.
    - If the match_part returns a match, then it sets the matched text to the last_part
    variable.
    - Then identifies the section type and creates a candidate using the section type
    and the semantic element.
    - Else if checks whether the elements text matches an item pattern by calling the
    match_item method.
    - If the match_item returns a match, then it identifies the section type and creates
    a candidate using
    ...
    def _identify_candidate(self, element: AbstractSemanticElement) -> None:
        ...
        if part := self.match_part(element.text):
            ...
            if section_type is InvalidTopSectionIn10Q:
                warnings.warn(
                    f"Invalid section type for part{self._last_part}. Defaulting to"
                    " InvalidTopSectionIn10Q.",
                    UserWarning,
                    stacklevel=8,
                )
            ...
            if section_type is InvalidTopSectionIn10Q:
                warnings.warn(
                    f"Invalid section type for part{self._last_part}item{item}."
                    " Defaulting to InvalidTopSectionIn10Q.",
                    UserWarning,
                    stacklevel=8,
                )
            candidate = _Candidate(section_type, element)
    ...
    """
    Returns the corresponding TopSectionType of the given identifier. The TopSectionType
    represents a standard top section type in the context of a 10-Q report.
    The function utilizes the IDENTIFIER_TO_10Q_SECTION dictionary.

    Input:
    - identifier (type: String): an identifier of a top section title expressed by a
    string

    Output:
    - returns the corresponding TopSectionType of the given identifier. Returns
    InvalisTopSectionIn10Q if the identifier doesn't match any TopSectionType.
    """
    ...
    """"
    Groups candidates by section type. Then selects the first element candidate of each
    section type by using the helper function select_element.

    Input: No input

    Output: returns a tuple of selected candidates. There should be a candidate for
    each section type.
    ...
    def _select_candidates(self) -> tuple[_Candidate, ...]:
        ...
        """
         ...
         Input:
        - elements (type: a list of AbstractSemanticElement): instances of the
        ...
        Functionlaity:
        - Checks if there is one element in the given input list. If yes, it returns the
        single element.
        - Removes elements containing the tag "table" from the input list.
        - If there are at least two elements in the modified list:
           a. Checks whether the first element in the list is a potential page header.
           b. If yes, returns the second element in the list.
        - If none of the conditions are met, the first element in the list is returned.
        """
        ...
        def select_element(
            ...
            if len(elements) == 1:
                return elements[0]
            # here the method has returned the element
            ...
            if len(elements_without_table) >= 1:
                elements = elements_without_table

            if len(elements) >= TWO_ELEMNTS_LENGTH:
                ...
                style: TextStyle = TextStyle.from_style_and_text(
                    styles_metrics, elements[0].text,
                )
                if (
                    elements[0].contains_words()
                    and style
                    and len(elements[0].text) <= PAGE_HEADER_TEXT_LENGTH_THRESHOLD
                ):
                    return elements[1]
    ...
    If yes, it updates the last order number, in case the order of the candidate is
    greater than current last order number.
    Then it creates a top section title of the element and returns the new top section
    title element.
    ...
    Output:
    - Either the original input element or a newly generated top section title element
    ...
    def _log_order_number_not_greater(
        ...
        message = f"Order number {order} is not greater than last order number"
...
"""
Algorithm:
1. Call process_element with semantic element and iteration context. The output should
be the processed semantic element.
2. Process the semantic element based on the given iteration number.
3. If the iteration number is 0, then the process_iteration_0 identifies whether the
given semantic element is a top section title canadidate
   And appends the element to the list of candidates if it qualifies as a top section
   title.
4. If the iteration number is 1, then it selects candidates for each section type. Then
it processes the selected candidates.
   By iterating over all the selected candidates and checking whether the current
   semantic element is in the list of selected candidates.
   If yes, then it either updates last order number and creates a top section title
   element and returns it. or logs order number not greater and continues
...
Begin by invoking the process_element function with a semantic element and the iteration
context. Capture the output as the processed semantic element.

Proceed to process the semantic element based on the given iteration number.

If the iteration number is 0:
a. Utilize the process_iteration_0 function to determine if the semantic element
qualifies as a top section title candidate.
...
If yes:
Update the last order number and returns the top section title element version of the
current element
 or log the order number if it's not greater and continues to scan the list of selected
 candidates.
