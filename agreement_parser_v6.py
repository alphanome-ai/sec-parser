#!/usr/bin/env python3
"""Legal Agreement Parser V6 - Incremental improvements
- Fix title detection for HTML align attributes (Agreement 6)
- Detect underlined headers as sections (Agreement 2)
- Better early title detection after metadata.
"""
...
class ImprovedMainTitleClassifier(AbstractElementwiseProcessingStep):
    ...
    def _is_main_title_v6(self, text: str, html_tag: HtmlTag, position: int) -> bool:
        ...
        # V6: Check HTML attributes (for Agreement 6)
        try:
            # Check align attribute
            if hasattr(html_tag, "_bs4"):
                align = html_tag._bs4.get("align", "").lower()
                is_centered_attr = align == "center"

                # Check if bold via child tags
                is_bold_tag = bool(html_tag._bs4.find("b") or html_tag._bs4.find("strong"))

                # Check style attribute
                style = html_tag._bs4.get("style", "")
                is_centered_style = "center" in style.lower()
                is_bold_style = "bold" in style.lower()

                # Any centering method + bold = strong title indicator
                is_centered = is_centered_attr or is_centered_style
