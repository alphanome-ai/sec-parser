...
class SecapioApiKeyGetter:
    ...
    def _ask_user_for_api_key(self):
        with self.st_container:
            ...
            msg = "**Note:** We suggest setting the `SECAPIO_API_KEY` environment variable, possibly in an `.env` file. This method allows you to utilize the API key in your code without the need for manual entry each time."
            container.info(msg)
            if not self.api_key:
                st.stop()
