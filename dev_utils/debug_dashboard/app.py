    ...
    if selected_step >= 2 and selected_step <= 3:
        with st.sidebar:
            ...
            with PassthroughContext():  # replace with st.expander("") if needed
                ...
                for elements in elements_lists:
                    elements[:] = [
                        e for e in elements if any(type(e) == t for t in selected_types)
                    ]
...
def to_tree_item(tree_node: sp.TreeNode, indexer):
    ...
    return sac.TreeItem(
        ...
        children=children,
        icon=icon,
        tag=f"{len(element.text)}" if show_text_length else str(index),
    )
...
if selected_step == 3 and use_tree_view:
    documents = tuple(
        k for k in zip_longest(elements_lists, htmls_urls, metadatas, fillvalue=None)
    )
    if len(documents) > 1:
    ...
    with left, st.expander("Browser", expanded=True):
        ...
        indexer = Indexer()
        tree_items = [to_tree_item(k, indexer) for k in list(tree)]
...
with st.sidebar:
    ...
    st.markdown(
        f"""
        <div style="position: fixed; bottom: 0; width: 100%; text-align: left; color: lightgrey;">
            {footer}
        </div>
        """,
        unsafe_allow_html=True,
